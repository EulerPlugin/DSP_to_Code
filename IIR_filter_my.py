...
#relative path to a single-channel audio file
habanera_path = 'habanera.wav'

# read the audio file
data, fs = sf.read(habanera_path)

# our filter is set up for 48kHz so we cannot process audio at other sample rates
assert fs == 48000

# prepare an array for the output of floating-point samples
output = np.zeros_like(data, dtype=np.float64)

# filter the signal
IIR_filter(data, output)

# save the output to the file
sf.write('IIR_Filtered_habanera.wav', output, fs)
